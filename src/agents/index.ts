...
import { createSuperBuildAgent } from './super-build';
import { createSuperPlanAgent } from './super-plan';
...
export function createAgents(config?: PluginConfig): AgentDefinition[] {
  ...
  // 3a. Create Super_Plan and Super_Build primary agents
  const primaryAgents: AgentDefinition[] = [];

  for (const primaryName of PRIMARY_AGENT_NAMES) {
    if (disabled.has(primaryName)) continue;

    const override = getAgentOverride(config, primaryName);
    const model = override?.model ?? DEFAULT_MODELS[primaryName];
    const customPrompts = loadAgentPrompt(primaryName, config?.preset);

    const factory =
      primaryName === 'super_plan'
        ? createSuperPlanAgent
        : createSuperBuildAgent;
  ...
  // Collect all display names from orchestrator, primary agents, and all subagents
  ...
  return [orchestrator, ...primaryAgents, ...allSubAgents];
}
...
export function getAgentConfigs(
  ...
  const applyClassification = (
    ...
    if (name === 'council') {
      ...
      name === 'orchestrator' ||
      (PRIMARY_AGENT_NAMES as readonly string[]).includes(name)
    ) {
