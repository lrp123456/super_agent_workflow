...
- Role: Chief Architect — surveys codebase, clarifies requirements, generates structured execution plans
- Permissions: Read files, question user, delegate to @explorer/@librarian/@oracle
- Stats: Produces rigorous, human-reviewed plans with acceptance criteria
- Capabilities: Code map assessment, requirement analysis, brainstorming, plan generation (final_plan.md + final_plan.json)
- **Delegate when:** User requests a complex feature that needs careful planning • Multi-step refactoring requiring human approval • Tasks where "measure twice, cut once" matters • User explicitly asks for a plan
- **Don't delegate when:** Simple bug fix or small change • Task is clear and bounded • Quick implementation is more efficient than planning overhead
- **Rule of thumb:** Complex feature needing human sign-off? → @super_plan. Just do it? → yourself or @fixer.`,

  super_build: `@super_build
- Role: Chief Engineering Director — executes approved plans with dual-review quality pipeline
- Permissions: Read/write files, delegate to @explorer/@librarian/@oracle/@fixer
- Stats: Enforces LSP syntax check + acceptance criteria verification for every task
- Capabilities: Plan parsing, parallel task dispatch, dual-review pipeline (LSP + logic), conflict resolution, code map update
- **Delegate when:** An approved plan exists (final_plan.json) and needs execution • User says "execute the plan" or "build it" • Complex multi-file changes requiring quality gates
- **Don't delegate when:** No plan exists yet • Simple single-file change • User wants interactive step-by-step work
- **Rule of thumb:** Have a plan and need it executed with quality gates? → @super_build. No plan? → @super_plan first.`,
};
