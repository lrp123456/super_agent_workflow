...
import {
  ...
  createClearContextCommand,
  createContextTransitionHook,
  createDelegateTaskRetryHook,
  createFilterAvailableSkillsHook,
  createJsonErrorRecoveryHook,
  createPhaseReminderHook,
  createPostFileToolNudgeHook,
  createSuperBuildWorkflowHook,
  createSuperPlanWorkflowHook,
...
import { createBuiltinMcps, McpFailoverManager, McpHealthMonitor } from './mcp';
import {
  getMultiplexer,
...
const OhMyOpenCodeLite: Plugin = async (ctx) => {
  ...
  let codemapTools: Record<string, unknown>;
  let planTools: Record<string, unknown>;
  let lspTools: Record<string, unknown>;
  let fixHistoryTools: Record<string, unknown>;
  let superPlanWorkflowHook: ReturnType<typeof createSuperPlanWorkflowHook>;
  let superBuildWorkflowHook: ReturnType<typeof createSuperBuildWorkflowHook>;
  let contextTransitionHook: ReturnType<typeof createContextTransitionHook>;
  let clearContextCommand: ReturnType<typeof createClearContextCommand>;
  let testResultCommand: ReturnType<typeof createTestResultCommand>;
  ...
  try {
    ...
    let mcpHealthMonitor: McpHealthMonitor | undefined;
    let mcpFailoverManager: McpFailoverManager | undefined;

    if (config.mcpFailover?.enabled) {
      mcpHealthMonitor = new McpHealthMonitor({
        timeoutMs: config.mcpFailover.timeoutMs,
        checkIntervalMs: config.mcpFailover.healthCheckIntervalMs,
        failureThreshold: config.mcpFailover.failureThreshold,
        recoveryThreshold: config.mcpFailover.recoveryThreshold,
      });

      const failoverConfig = {
        enabled: true,
        chains: Object.fromEntries(
          Object.entries(config.mcpFailover.chains).map(([key, chain]) => [
            key,
            {
        ...
        autoRecovery: config.mcpFailover.autoRecovery ?? true,
        recoveryCooldownMs: config.mcpFailover.recoveryCooldownMs ?? 300_000,
      };

      mcpFailoverManager = new McpFailoverManager(
        failoverConfig,
        mcpHealthMonitor,
      );

      mcps = mcpFailoverManager.registerChains(mcps);

      mcpHealthMonitor.start(mcps, (name, oldStatus, newStatus) => {
        if (!mcpFailoverManager) return;
        const newConfig = mcpFailoverManager.handleStatusChange(
          name,
          oldStatus,
          newStatus,
        );
        if (newConfig) {
          mcps[name] = newConfig;
          log('[plugin] MCP failover applied', {
            service: name,
            activeService: mcpFailoverManager.getActiveService(name),
          });
        }
      });

      log('[plugin] MCP failover enabled', {
        chains: Object.keys(failoverConfig.chains),
      });
    }
    webfetch = createWebfetchTool(ctx);
    codemapTools = createCodemapTools(ctx);
    planTools = createPlanTools(ctx);
    lspTools = createLspTools(ctx);
    fixHistoryTools = createFixHistoryTools(ctx);
    ...
    // Initialize super_plan workflow hook (codemap check nudge)
    superPlanWorkflowHook = createSuperPlanWorkflowHook(ctx, config);

    // Initialize super_build workflow hook (codemap auto-update)
    superBuildWorkflowHook = createSuperBuildWorkflowHook(ctx, config);

    // Initialize context transition hook (Plan→Build context wipe)
    contextTransitionHook = createContextTransitionHook(ctx, config);

    // Initialize /clear_context command handler
    clearContextCommand = createClearContextCommand(ctx, config);

    // Initialize /test_result command handler
    testResultCommand = createTestResultCommand(ctx, config);
    ...
    toolCount =
      ...
      Object.keys(codemapTools).length +
      Object.keys(planTools).length +
      Object.keys(lspTools).length +
      Object.keys(fixHistoryTools).length +
      1 + // webfetch
  ...
  return {
    ...
    'command.execute.before': async (input, output) => {
      ...
      await clearContextCommand.handleCommandExecuteBefore(
        input as {
          command: string;
          sessionID: string;
          arguments: string;
      ...
      await testResultCommand.handleCommandExecuteBefore(
        input as {
          command: string;
          sessionID: string;
          arguments: string;
    ...
    'experimental.chat.messages.transform': async (
      ...
      await superPlanWorkflowHook['experimental.chat.messages.transform'](
        input,
        typedOutput,
      );
      await superBuildWorkflowHook['experimental.chat.messages.transform'](
        input,
        typedOutput,
      );
      await contextTransitionHook['experimental.chat.messages.transform'](
        input,
        typedOutput,
      );
    ...
    'tool.execute.after': async (input, output) => {
      ...
      await superBuildWorkflowHook['tool.execute.after'](
...
export type {
  McpFailoverChain,
  McpFailoverConfig as McpFailoverConfigType,
  RemoteMcpConfig,
} from './mcp';
