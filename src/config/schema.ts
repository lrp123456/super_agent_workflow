...
export const SuperPlanConfigSchema = z.object({
  maxQuestions: z
    .number()
    .int()
    .min(1)
    ...
    .default(4)
    .describe(
      'Maximum number of clarification questions super_plan will ask before generating a plan',
    ),
  brainstormEnabled: z
    .boolean()
    .default(true)
    .describe(
      'Enable brainstorming phase in super_plan to generate multiple candidate approaches',
    ),
});

export type SuperPlanConfig = z.infer<typeof SuperPlanConfigSchema>;

export const SuperBuildConfigSchema = z.object({
  maxRetries: z
    .number()
    .int()
    .min(1)
    .max(10)
    .default(3)
    .describe('Maximum number of retry attempts per task when review fails'),
  taskTimeoutMs: z
    .number()
    .int()
    .min(10_000)
    .max(600_000)
    .default(300_000)
    .describe('Timeout in milliseconds for each individual task execution'),
  parallelTasks: z
    .number()
    .int()
    .min(1)
    .max(10)
    .default(3)
    .describe('Maximum number of tasks to execute in parallel'),
  lspChecker: z
    .enum(['auto', 'tsc', 'biome', 'both'])
    .default('auto')
    .describe('Which syntax checker to use in the LSP review gate'),
});

export type SuperBuildConfig = z.infer<typeof SuperBuildConfigSchema>;

export const CodemapConfigSchema = z.object({
  enabled: z
    .boolean()
    .default(true)
    .describe('Enable code map generation and maintenance'),
  autoUpdate: z
    .boolean()
    .default(true)
    .describe(
      'Automatically update the code map after super_build completes all tasks',
    ),
});

export type CodemapConfig = z.infer<typeof CodemapConfigSchema>;

const McpFailoverChainSchema = z.object({
  primary: z.string().describe('Primary MCP service name'),
  fallbacks: z
    .array(z.string())
    .default([])
    .describe('Ordered list of fallback MCP service names'),
});

export const McpFailoverConfigSchema = z.object({
  enabled: z
    .boolean()
    .default(false)
    .describe('Enable MCP service health monitoring and automatic failover'),
  healthCheckIntervalMs: z
    ...
    .max(600_000)
    .default(60_000)
    .describe('Interval in milliseconds between health checks'),
  failureThreshold: z
    .number()
    .int()
    .min(1)
    ...
    .default(3)
    .describe(
      'Number of consecutive health check failures before switching to fallback',
    ),
  recoveryThreshold: z
    .number()
    .int()
    ...
    .default(2)
    .describe(
      'Number of consecutive health check successes before recovering to primary',
    ),
  timeoutMs: z
    .number()
    .int()
    ...
    .max(60_000)
    .default(10_000)
    .describe('Timeout in milliseconds for each health check request'),
  autoRecovery: z
    .boolean()
    .default(true)
    .describe(
      'Automatically recover to primary service when it becomes healthy again',
    ),
  recoveryCooldownMs: z
    .number()
    .int()
    .min(0)
    .max(3_600_000)
    .default(300_000)
    .describe(
      'Minimum time in milliseconds before attempting recovery to primary',
    ),
  chains: z
    .record(z.string(), McpFailoverChainSchema)
    .default({})
    .describe(
      'Failover chains per MCP service. Key is the logical service name, ' +
        'value defines the primary and fallback services.',
    ),
});

export type McpFailoverConfig = z.infer<typeof McpFailoverConfigSchema>;
...
export const PluginConfigSchema = z
  .object({
    ...
    superPlan: SuperPlanConfigSchema.optional(),
    superBuild: SuperBuildConfigSchema.optional(),
    codemap: CodemapConfigSchema.optional(),
    mcpFailover: McpFailoverConfigSchema.optional(),
  })
  .superRefine((value, ctx) => {
    if (value.agents) {
